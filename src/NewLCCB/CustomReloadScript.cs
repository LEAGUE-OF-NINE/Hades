...
namespace BaseMod
{
    internal class CustomReloadScript : MonoBehaviour
    {
        public static void Setup(Harmony harmony)
        {
            ClassInjector.RegisterTypeInIl2Cpp<CustomReloadScript>();
            harmony.PatchAll(typeof(CustomReloadScript));
        }
        ...
        private static void OnStartTurn_AfterLog(BattleActionModel action, BattleUnitModel __instance, BATTLE_EVENT_TIMING timing)
        {
            foreach (var ability in action._skill.GetSkillAbilityScript())
            {
                ...
                if (scriptName.Contains("CustomReloadScript_"))
                {
                    // if buffdata is empty, skip this.
                    if (ability.buffData == null) continue;

                    var name_keyword = scriptName.Replace("CustomReloadScript_", "");
                    ...
                    //  (Reload: If this unit has less than [value] [keyword], regain([value] - (Current [keyword] Amount)) [keyword]
